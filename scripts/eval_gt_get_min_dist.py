"""
get min distance between two closest pair of peds in each dataset
"""
import os
import argparse
...
def get_collisions_mat_old(sample_idx, pred_traj_fake, threshold):
    ...
    return sample_idx, np.any(np.any(collision_mat, axis=0), axis=0), collision_mat_vals  # collision_mat_pred_t_bool
...
def compute_CR(pred_arr,
    ...
    if n_ped > 1:
        ...
        for i, pa in enumerate(pred_arr):
            ...
            # print(time.time() - start)
            # import ipdb; ipdb.set_trace()
            tup = get_collisions_mat_old(i, pa, threshold)
            r.append(tup)
    ...
    cr_pred *= 100
    return cr_pred / n_ped, r[-1][-1] if n_ped > 1 else np.inf #None
...
def eval_one_seq(gt_eval, stats_meter, stats_func, obs_traj=None, i=None):
    ...
    return values, minn#agent_traj_nums
...
def main(args):
    ...
    log = open(os.path.join('viz', '../temp.txt'), 'a+')
    ...
    if args.multiprocess:
        with Pool() as pool:
            ...
            print("all_meters_agent_traj_nums:", np.min(all_meters_agent_traj_nums))
            return
...
if __name__ == '__main__':
    ...
    for cfg in ['eth', 'hotel', 'zara1', 'zara2', 'univ']:
        args.cfg = f'{cfg}_agentformer'
        main(args)
