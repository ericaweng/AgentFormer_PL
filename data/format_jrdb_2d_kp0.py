""" Format JRDB 2D body keypoints into AgentFormer-consumable format. try 0"""
...
def load_bboxes_and_poses(scene_name_w_image, args):
    ...
    for image_id, annos in sorted(image_id_to_pose_annos.items(), key=lambda x: x[0]):
        ...
        for ann in annos:
            ...
            assert np.all(
                    [np.isclose(p, 0.1) or np.isclose(p, 0.5) or np.isclose(p, 1) for p in score]), f"score: {score}"
        ...
        for track_id, box in all_bboxes[image_id].items():
            ...
            # elif False and 'attributes' not in box or 'occlusion' not in box['attributes']
            # or 'visible' in box['attributes']['occlusion']:  # only want boxes that are Fully Visible or
            # Partially Visible (not Occluded)
            #     poses_this_frame[track_id] = {'box': box_to_2d_corners(box['box'])}
...
def main():
        ...
        for frame, poses in this_scene_poses.items():
            ...
            for ped_id, kp in poses.items():
                combined_poses[scene][frame][ped_id] = kp[
                    'kp']  # {'kp': kp, 'cam_id': 0, 'intrinsics': intrinsic_params[0]}
    ...
    counts = {'num_scenes': 0, 'num_frames': 0, 'num_unique_peds': 0, 'ped_id_to_set': set(),
              'num_each_cam_id': {0: 0, 2: 0, 4: 0, 6: 0, 8: 0}}
