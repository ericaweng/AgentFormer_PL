...
from data.interactions_helper import (check_int, compute_dist_rel, compute_speed, interaction_length_gt,
                                      interaction_length_lt, find_sublist)
...
INTERACTION_INDEX = {cat: i for i, cat in enumerate(INTERACTION_CAT_NAMES)}
INTERACTION_CAT_ABBRS = list(INTERACTION_CAT_NAMES.keys())
N_CATEGORIES = len(INTERACTION_CAT_ABBRS)
...
INTERACTION_HPARAMS = {
    ...
    # Threshold for significant speed change detection
    "sc_speed_diff_min": 1.0,  # Min speed difference to detect significant change

    # Thresholds for classifying non-linear movement
    "nl_resid_min": 0.2,  # Min residual threshold for non-linear classification (higher means less likely to be nonlinear)
    "nl_degree": 2.0,  # Degree for non-linear movement classification

    # Thresholds for group behavior classification
    "grp_mean_rel_dist_max": 1.0,  # Max mean relative distance for group classification
    "grp_std_rel_dist_max": 0.2,  # Max standard deviation of relative distance for group classification

    # Thresholds for leader-follower behavior classification
    "lf_dist_max": 3.0,  # Distance threshold for classifying leaders and followers (in meters)
    "lf_speed_diff_max": 1.0,  # Max speed difference for leader-follower classification

    # Thresholds for avoidance behaviors
    "avoidance_dist_max": 1.0,  # Distance threshold for avoidance behavior (in meters)
    "ma_dist_max": 1.0,  # Distance threshold for mutual avoidance (in meters)
    "avoidance_max_time_within_dist": 4  # Max time agents should be within avoidance_dist_max of each other
}
...
def is_moving_to_static(path, static_speed_max=INTERACTION_HPARAMS['static_speed_max'], moving_speed_min=INTERACTION_HPARAMS['moving_speed_min']):
...
def is_static_to_moving(path, static_speed_max=INTERACTION_HPARAMS['static_speed_max'], moving_speed_min=INTERACTION_HPARAMS['moving_speed_min']):
...
def is_static_and_moving(path, static_speed_max=INTERACTION_HPARAMS['static_speed_max'], moving_speed_min=INTERACTION_HPARAMS['moving_speed_min']):
...
def is_speed_change(path, sc_speed_diff_min=INTERACTION_HPARAMS['sc_speed_diff_min']):
...
def is_linear(path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'], static_speed_max=INTERACTION_HPARAMS['static_speed_max'],
              nl_resid_min=INTERACTION_HPARAMS['nl_resid_min'], nl_degree=INTERACTION_HPARAMS['nl_degree']):
...
def is_non_linear(path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'], static_speed_max=INTERACTION_HPARAMS['static_speed_max'],
                  nl_resid_min=INTERACTION_HPARAMS['nl_resid_min'], nl_degree=INTERACTION_HPARAMS['nl_degree']):
    is_non_linear_vec, info = _non_linear(path, nl_resid_min, nl_degree)
    return (~is_static(path, static_dist_max, static_speed_max)[0]
            & is_non_linear_vec).item(), info
...
def is_group(path, neigh_path, grp_mean_rel_dist_max=INTERACTION_HPARAMS['grp_mean_rel_dist_max'],
             grp_std_rel_dist_max=INTERACTION_HPARAMS['grp_std_rel_dist_max']):
...
def _is_group_dist(path, neigh_path, grp_mean_rel_dist_max, grp_std_rel_dist_max):
    """whether ego-agent is part of group with other agents,
    based on mean dist from agent over the course of the traj"""
...
def is_leader_follower(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'],
                       static_speed_max=INTERACTION_HPARAMS['static_speed_max'], lf_dist_max=INTERACTION_HPARAMS['lf_dist_max']):
...
def is_leader(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'], static_speed_max=INTERACTION_HPARAMS['static_speed_max'],
              lf_dist_max=INTERACTION_HPARAMS['lf_dist_max'], lf_speed_diff_max=INTERACTION_HPARAMS['lf_speed_diff_max']):
    """ Identifying Leader Behavior """
...
def is_follower(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'], static_speed_max=INTERACTION_HPARAMS['static_speed_max'],
                lf_dist_max=INTERACTION_HPARAMS['lf_dist_max'], lf_speed_diff_max=INTERACTION_HPARAMS['lf_speed_diff_max']):
    """ Identifying Follower Behavior """
...
def is_passing(path, neigh_path, avoidance_dist_max=INTERACTION_HPARAMS['avoidance_dist_max'],
               static_dist_max=INTERACTION_HPARAMS['static_dist_max'],
               static_speed_max=INTERACTION_HPARAMS['static_speed_max']):
...
def is_avoidance(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'],
                 static_speed_max=INTERACTION_HPARAMS['static_speed_max'], avoidance_dist_max=INTERACTION_HPARAMS['avoidance_dist_max'],
                 moving_speed_min=INTERACTION_HPARAMS['moving_speed_min']):
...
def is_collision_avoidance(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'],
                           static_speed_max=INTERACTION_HPARAMS['static_speed_max']):
...
def is_mutual_avoidance(path, neigh_path, static_dist_max=INTERACTION_HPARAMS['static_dist_max'],
                        static_speed_max=INTERACTION_HPARAMS['static_speed_max'],
                        avoidance_dist_max=INTERACTION_HPARAMS['avoidance_dist_max'], moving_speed_min=INTERACTION_HPARAMS['moving_speed_min']):
    """ Identifying Collision Avoidance Behavior """
    interaction_conds_met = check_int(path[8:],
                                      neigh_path[8:],
                                      pos_range=100,
...
INTERACTION_CAT_FNS = [globals()[f'is_{name.replace(" ", "_").replace("-", "_")}'] for abbr, name in INTERACTION_CAT_NAMES.items()]

INTERACTION_CAT_TO_FN = {
    a: b
    for a, b in zip(INTERACTION_CAT_ABBRS, INTERACTION_CAT_FNS)
}