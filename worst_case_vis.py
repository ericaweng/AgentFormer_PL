...
def main(args):
    ...
    for frame_path_ in all_frames[::skip]:
        ...
        other_mSFDE, other_mSADE, other_mFDE, other_mADE, other_maxSADE = [], [], [], [], []
        non_ours_args_list = []
        ...
        for method_i, method in enumerate(args.method):
            ...
            if method == 'trajectron':
                diff = obs_traj - res[-1]
                assert np.allclose(diff, diff[0,0], atol=1e-2), f"diff: {diff} vs. {diff[0,0]}"
            ...
            maxSADE_i, maxSADE = np.argmax(sample_metrics['SADE']), np.max(sample_metrics['SADE'])
            ...
            if args.refine and method == OURS and not (
                   ...
                   # np.sum(other_mSADE < mSADE) > 4
                   np.all(other_maxSADE > maxSADE)
            ...
            if method != OURS:
                ...
                pass
            selected_samples = np.argsort(sample_sades)[-NUM_SAMPLES:]
            # selected_samples = np.array([maxSADE_i])
            ...
            for subplot_i, pred_traj in enumerate(to_plot_pred_fake_traj):
                ...
                args_dict = {'plot_title': f"{METHOD_DISP_NAMES[method]}" if subplot_i == MIDDLE_SUBPLOT_I else "",#f'{mADE:0.2f}',#f"" if sample_i == 2 and method == 'agentformer' else f"{method} {sample_i}",
                             ...
                             'is_best_JADE': False,#subplot_i == best_JADE_selected,#sade_argmin,
                             ...
                             'is_best_JADE_of_all': False,#True if method == OURS else False,
            ...
            if method != OURS:
                ...
                best_JADE_method_i = np.argmin(other_mSADE) #len(args.method) - 1
                non_ours_args_list[best_ADE_method_i][MIDDLE_SUBPLOT_I]['is_best_ADE_of_all'] = True
                non_ours_args_list[best_JADE_method_i][MIDDLE_SUBPLOT_I]['is_best_JADE_of_all'] = True
                ...
                non_ours_args_list[best_JADE_method_i][MIDDLE_SUBPLOT_I]['plot_title'] = f"{METHOD_DISP_NAMES[args.method[best_JADE_method_i]]} (best JADE)"
                # args_list[MIDDLE_SUBPLOT_I]['plot_title'] = f"{METHOD_DISP_NAMES[args.method[best_JADE_method_i]]}\n(best JADE)"
                plot_args = [anim_save_fn, "", bounds, grid, METHODS_ON_TOP, 6.5, additional_y, [*non_ours_args_list, args_list]]
