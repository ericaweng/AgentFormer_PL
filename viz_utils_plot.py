...
from visualization_scripts.viz_utils_univ import plot_anim_grid
...
from visualization_scripts.viz_utils_univ import AnimObjBEVTraj2d
...
def _save_catch_all(self, outputs, all_sample_vals, collision_mats, tag=''):
    seq_to_plot_args = []
    for frame_i, (output, seq_to_sample_metrics) in enumerate(zip(outputs, all_sample_vals)):
        frame = output['frame']
        seq = output['seq']
        ...
        args_dict = {'plot_title': f"best mSADE sample",
                     ...
                     'collision_mats': collision_mats[frame_i][-1],
                     'text_fixed': min_SADE_stats}
        if "heading" in self.cfg.id or output['data']['heading_vec'].detach().cpu().numpy() is not None:
            heading = output['data']['heading_vec'].detach().cpu().numpy()  # (1,2)
            heading_avg = output['data']['heading_avg'].detach().cpu().numpy()  # (1,2)
...
def _save_viz_gt(outputs, args, tag):
    ...
    if args.mp:
        with multiprocessing.Pool(multiprocessing.cpu_count()) as pool:
            all_figs = pool.apply_async(plot_anim_grid, seq_to_plot_args)
