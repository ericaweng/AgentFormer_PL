    ...
    for frame_path_ in all_frames:
        ...
        for method_i, method in enumerate(args.method):
            ...
            if method != OURS:
                ...
                if method == 'memonet':
                    selected_samples = np.array([0, 1, 3, 4, 5])[-NUM_SAMPLES:]
                elif method == 'pecnet':
                    selected_samples = np.array([1, 2, 4, 7, 13])[-NUM_SAMPLES:]
                elif method == 'trajectron':
                    selected_samples = np.array([0, 1, 3, 4, 7])[-NUM_SAMPLES:]
                elif method == 'ynet':
                    selected_samples = np.array([0, 1, 3, 4, 8, 9])[-NUM_SAMPLES:]
                elif method == 'agentformer':
                    selected_samples = np.array([0, 1, 3, 13, 16, 17])[-NUM_SAMPLES:]
                elif method == 'sgan':
                    selected_samples = np.array([4, 13, 14, 15, 19])[-NUM_SAMPLES:]
                ...
                if args.refine and not at_least_one_method_has_cols:
                    ...
                    break
                selected_samples = np.array([0,1,2,3,5])[:NUM_SAMPLES]
                pass
            ...
            if method != OURS:
                ...
                plot_args = [anim_save_fn, "", bounds, (NUM_SAMPLES, len(args.method)), *non_ours_args_list, *args_list]
            ...
            if args.plot_online and len(seq_to_plot_args) > 0:
                ...
                plot_img_grid_old(*OURS_plot_args_list)
